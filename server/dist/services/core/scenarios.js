...
import { z } from "zod";
export async function generateScenarios(ticker, snapshot, companyName) {
    const prompt = `
  ...
  `;
    const ScenariosSchema = z.array(z.object({
        ...
        revenueGrowthAssumption: z.number().optional(),
        marginAssumption: z.number().optional(),
        valuationMultipleAssumption: z.number().optional(),
        riskAssumption: z.string().optional(),
        reasoning: z.string().optional()
    }));
    try {
        const parsed = await callLLMWithSchema(prompt, ScenariosSchema, 0.3);
