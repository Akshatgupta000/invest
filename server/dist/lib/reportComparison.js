...
exports.compareWithPreviousReport = compareWithPreviousReport;
const Report_1 = __importDefault(require("../models/Report"));
async function compareWithPreviousReport(newReport) {
    ...
    try {
        ...
        const previousReport = await Report_1.default.findOne({
        ...
        const prevRisks = new Set();
        if (previousReport.agentOutputs?.risk?.concerns) {
            previousReport.agentOutputs.risk.concerns.forEach((c) => prevRisks.add(c));
        }
        else if (previousReport.agentDebate?.riskAgent?.redFlags) {
            previousReport.agentDebate.riskAgent.redFlags.forEach((c) => prevRisks.add(c));
        }
        ...
        const prevDrivers = new Set();
        if (previousReport.agentOutputs?.bull?.arguments) {
            previousReport.agentOutputs.bull.arguments.forEach((a) => prevDrivers.add(a.claim));
        }
        else if (previousReport.agentDebate?.bullAgent?.arguments) {
            previousReport.agentDebate.bullAgent.arguments.forEach((a) => prevDrivers.add(a));
        }
        ...
        return {
            ...
            removedRisks,
            newPositiveDrivers,
            summary: `Compared to the previous report from ${previousReport.createdAt.toLocaleDateString()}, ${scoreChange ? (scoreChange > 0 ? 'the score improved.' : 'the score declined.') : 'the verdict was evaluated.'}`
        };
    }
    catch (err) {
