...
exports.identifyCompetitors = identifyCompetitors;
exports.fetchCompetitorData = fetchCompetitorData;
exports.buildPeerComparison = buildPeerComparison;
const finance_1 = require("./finance");
const llm_1 = require("./llm");
const zod_1 = require("zod");
async function identifyCompetitors(company, ticker, sector, industry) {
    const prompt = `
  ...
  `;
    try {
        const parsed = await (0, llm_1.callLLMWithSchema)(prompt, zod_1.z.array(zod_1.z.string()), 0.1);
...
async function fetchCompetitorData(tickers) {
    const competitors = [];
    for (const ct of tickers) {
        try {
            const cdata = await (0, finance_1.fetchFinancialData)(ct);
            competitors.push({
                ticker: cdata.ticker,
                name: cdata.company,
...
function buildPeerComparison(target, peers) {
    const benchmarks = [];
