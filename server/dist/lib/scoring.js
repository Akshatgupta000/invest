...
exports.calculateAdvancedScores = calculateAdvancedScores;
function calculateAdvancedScores(snapshot, riskProfile, agentOutputs, competitors) {
    const categoryScores = [];
    const clamp = (val) => Math.min(100, Math.max(0, val));
    // 1. Valuation
    let valuationScore = 50;
    const valuationPositives = [];
    const valuationNegatives = [];
    if (snapshot.peRatio !== null) {
        if (snapshot.peRatio < 15) {
            valuationScore += 25;
            valuationPositives.push("Attractive P/E ratio.");
        }
        else if (snapshot.peRatio > 40) {
            valuationScore -= 20;
            valuationNegatives.push("High P/E ratio.");
        }
    }
    if (agentOutputs.valuation?.scoreBias) {
        valuationScore += agentOutputs.valuation.scoreBias * 0.5; // Agent bias influences score
        if (agentOutputs.valuation.scoreBias > 20)
            valuationPositives.push("Valuation agent sees upside.");
        if (agentOutputs.valuation.scoreBias < -20)
            valuationNegatives.push("Valuation agent warns of overvaluation.");
    }
    categoryScores.push({
        category: "Valuation",
    ...
    // 2. Financial Health
    let healthScore = 50;
    const healthPositives = [];
    const healthNegatives = [];
    if ((snapshot.profitMargin ?? 0) > 0.1) {
        healthScore += 20;
        healthPositives.push("Strong net margin.");
    }
    if ((snapshot.debtToEquity ?? 0) > 1.5) {
        healthScore -= 20;
        healthNegatives.push("High debt burden (D/E > 1.5x).");
    }
    if ((snapshot.currentRatio ?? 0) > 1.2) {
        healthScore += 10;
        healthPositives.push("Good short-term liquidity.");
    }
    categoryScores.push({
    ...
    // 3. Growth Potential
    let growthScore = 50;
    const growthPositives = [];
    const growthNegatives = [];
    if ((snapshot.revenueGrowth ?? 0) > 0.1) {
        growthScore += 20;
        growthPositives.push("Double-digit revenue growth.");
    }
    if ((snapshot.revenueGrowth ?? 0) < 0) {
        growthScore -= 20;
        growthNegatives.push("Declining revenue.");
    }
    if (agentOutputs.bull?.scoreBias > 0) {
        growthScore += agentOutputs.bull.scoreBias * 0.3;
    }
    categoryScores.push({
    ...
    competitors.forEach(comp => {
        if (comp.percentile >= 75) {
            peerScore += 15;
            peerPositives.push(`Top quartile in ${comp.category}.`);
        }
        if (comp.percentile <= 25) {
            peerScore -= 15;
            peerNegatives.push(`Bottom quartile in ${comp.category}.`);
        }
    });
    categoryScores.push({
    ...
    if (riskProfile === "conservative") {
        ...
        if (healthScore < 40)
            riskAdjustment -= 15; // Severe penalty for weak health
        if (agentOutputs.risk?.scoreBias < -50)
            riskAdjustment -= 15; // Severe penalty for identified risks
        ...
        if (growthScore > 75)
            riskAdjustment += 15; // Bonus for high growth
    ...
    let verdict = "PASS";
    if (finalScore >= thresholdUsed)
        verdict = "INVEST";
    else if (finalScore >= thresholdUsed - 15)
        verdict = "WATCHLIST";
    return {
        finalScore,
        verdict,
