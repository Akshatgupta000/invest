import { CompetitorData, CompetitorBenchmark } from "../../utils/types/research";
...
export async function fetchCompetitorData(tickers: string[]): Promise<CompetitorData[]> {
...
export function buildPeerComparison(target: CompetitorData, peers: CompetitorData[]): CompetitorBenchmark[] {
  const benchmarks: CompetitorBenchmark[] = [];
  const all = [target, ...peers];

  const rankMetric = (
    category: string,
    metricAccessor: (c: CompetitorData) => number | null,
    higherIsBetter: boolean,
    explanationBuilder: (targetRank: number, total: number, isGood: boolean) => string
  ) => {
    const valid = all.filter((c) => metricAccessor(c) !== null && metricAccessor(c) !== undefined);
    if (valid.length < 2) return; // Not enough peers for comparison
  ...
  rankMetric(
    "Valuation (P/E)",
    (c) => c.peRatio,
    false, // lower PE is better for value
